...
def validate_compose(
    *,
    ...
    console.print("[cyan]docker compose up -d --build[/cyan]")
    up = docker_runner.run(
        settings, ["compose", "up", "-d", "--build"], cwd=repo_dir,
    ...
    try:
        if host_port:
            ...
            time.sleep(15)
            ps = docker_runner.run(settings, ["compose", "ps", "-q"], cwd=repo_dir, timeout=10)
            if ps.stdout.strip():
        ...
        docker_runner.run(settings, ["compose", "down", "-v"], cwd=repo_dir, timeout=60, capture=True)


def _discover_app_port(settings: Settings, repo_dir: str, container_port: int) -> int | None:
    res = docker_runner.run(
        settings, ["compose", "port", "app", str(container_port)],
...
def _compose_logs(settings: Settings, repo_dir: str, n: int = 80) -> str:
    res = docker_runner.run(
        settings, ["compose", "logs", "--tail", str(n)],
