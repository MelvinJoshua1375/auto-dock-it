...
- Lottie animations now render through the `@lottiefiles/lottie-player` web component inside `st.components.v1.html`, with `unpkg` and `jsdelivr` tried in order. The `streamlit-lottie` Python package was removed from `requirements.txt`; its bundled frontend assets fail to load on Streamlit Cloud for some accounts and the failure happens inside the component iframe where Python cannot retry.
- Preview-mode banner separator changed from a hyphen to a colon and the editable-install step now appears in the body: `Clone the repo, install with pip install -e ., and run autodock run <url> locally`. Following the previous copy literally dropped users into "command not found".

### Fixed
- Code-block panels in light mode: every CSS rule targeted `data-testid="stCodeBlock"` while Streamlit 1.42+ ships `data-testid="stCode"`. Replaced the testid across all 50 occurrences and swapped the Pygments token classes (`.k`, `.kn`, `.s`) for the Prism.js classes Streamlit actually emits (`.token`, `.token.keyword`, `.token.string`, ...). The dark `#0d0d0d` terminal surface and the per-line transparency rules now reach the DOM in both themes.
- Inline backtick code in light mode no longer renders as a black bar with invisible text. The chip background was `#0d0d0d` and the text colour was also resolving to `#111111` from `var(--adi-text)`; pinned the text to `#f5f5f5` so the dark terminal chip stays readable.
- Streamlit's internal session-error modal ("Bad message format / Tried to use SessionInfo before it was initialized") is rewritten into plain English on sight. A zero-height `components.html` iframe whose script reaches into `parent.document` installs a `MutationObserver` that swaps the title to "Reconnecting to the app" and the body to "The server just restarted. Refresh the page in a few seconds and you should be back."
